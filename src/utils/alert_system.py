...
class AlertSystem:
    def __init__(self, config):
        ...
        alert_cfg = config.get('logging', {}).get('alert_system', {})
        self.voice_alerts_enabled = alert_cfg.get('voice_alerts', True)
        ...
        if self.voice_alerts_enabled:
            try:
                ...
                pygame.mixer.music.set_volume(alert_cfg.get('alert_volume', 0.8))
            except Exception as e:
                print(f"Voice alerts disabled: {e}")
    ...
    def speak_alert(self, alert_type):
        ...
        if not self.voice_alerts_enabled or not self.audio_available:
            return
